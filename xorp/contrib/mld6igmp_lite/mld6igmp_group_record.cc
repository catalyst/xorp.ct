		...
		const IPvX& last_reported_host)
{
	...
	if (! sources.empty()) 
	{
		XLOG_WARNING("Non-empty list of sources when processing mode is EXCLUDE");
	}
		...
		const IPvX& last_reported_host)
{
	...
	if (! sources.empty()) 
	{
		XLOG_WARNING("Non-empty list of sources when processing change to EXCLUDE");
	}
		...
		const IPvX& last_reported_host)
{
	...
	XLOG_ASSERT(group_record != NULL);
	group_record->process_mode_is_exclude(nosources, last_reported_host);
 ...
 */
uint32_t
Mld6igmpGroupRecord::igmpv1_host_present_timer_timeout_sec() const
{
	TimeVal tv;

	_igmpv1_host_present_timer.time_remaining(tv);
 ...
 */
uint32_t
Mld6igmpGroupRecord::igmpv2_mldv1_host_present_timer_timeout_sec() const
{
	TimeVal tv;

	_igmpv2_mldv1_host_present_timer.time_remaining(tv);
