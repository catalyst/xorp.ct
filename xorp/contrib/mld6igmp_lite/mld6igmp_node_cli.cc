...
Mld6igmpNodeCli::cli_show_mld6igmp_group(const vector<string>& argv)
{
	...
	for (uint32_t i = 0; i < mld6igmp_node().maxvifs(); i++) 
	{
				...
				++group_iter) 
		{
			...
			cli_print(c_format("%-6s %-15s %-15s %-12s %7d %1d %2s %5d %5d\n",
						mld6igmp_vif->name().c_str(),
						...
						version, state.c_str(),
						XORP_INT_CAST(group_record->igmpv1_host_present_timer_timeout_sec()),
						XORP_INT_CAST(group_record->igmpv2_mldv1_host_present_timer_timeout_sec())
						));
