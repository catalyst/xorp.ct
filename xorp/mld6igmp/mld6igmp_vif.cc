...
Mld6igmpVif::start(string& error_msg, const char* dbg)
{
	...
	wants_to_be_started = true;
	XLOG_WARNING("WARNING:  Delaying start of mld6igmp-vif: %s because underlying vif is not multicast capable.",
		     name().c_str());
	return XORP_OK;
    }
    ...
    //
    if (is_loopback()) {
	error_msg =  "mld6igmp: Loopback interfaces cannot be used for multicast.";
	return (XORP_ERROR);
    }
